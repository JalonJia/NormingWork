...
def upgrade_files(s_VBCode_path, s_compile_to) :
    ...
    #replace from *.vbp
    s_from_list = [r'MinorVer=.?', r'(.*)NP64A(.*)', r'CompatibleMode=".?"', r'Path32="(.*)"']
    s_to_list = ['MinorVer=6', r'\1NP66A\2', 'CompatibleMode="0"', f'Path32="{s_compile_to}"' ]
    s_to_list_eng = ['MinorVer=6', r'\1NP66A\2', 'CompatibleMode="0"', f'Path32="{s_compile_to}\\\\ENG"']
    ...
    for root, dirs, files in os.walk(s_VBCode_path): #files会得到目录下的文件（不包括文件夹）；dirs会获取到每个文件夹下面的子目录；root会遍历每个子文件夹
        ...
        for file in files:
            ...
            elif (s_filetype == '.vbp') and (len(s_filename) >= len('ACCPACNP0000')) :                
                #print(f'Current File: {s_filepath}')
...
if __name__ == '__main__' :
    #sPath = r"D:\Sage300\NP66A"
    sPath2 = r"D:\\Sage300\\NP66A"
    s_vb_home = r'C:\Program Files (x86)\Microsoft Visual Studio\VB98'
    s_UI_home = r'D:\Pluswdev\NP66A\Source'
    s_View_home = r'D:\Pluswdev\NP66A\Source\Cprogram'
